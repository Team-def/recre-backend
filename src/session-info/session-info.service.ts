...
export class SessionInfoService {
    ...
    // 무궁화 꽃이 방 생성
    redGreenGameSave(redGreenGameRoom: RedGreenGame) {
        return this.redGreenGameRepository.save(redGreenGameRoom);
    }

    // 무궁화 방 조회 (방 아이디로)
    redGreenGameFindByRoomId(room_id: number) {
        return this.redGreenGameRepository.findOne({
            where: { room_id },
        });
    }

    catchGamePlayerFindByUuid(uuid: string) {
    ...
    // 캐치마인드 플레이어 생성
    catchGamePlayerInsert(player: CatchPlayer) {
        return this.catchPlayerRepository.insert(player);
    }

    // 캐치마인드 플레이어 저장
    catchGamePlayerSave(player: CatchPlayer) {
        return this.catchPlayerRepository.save(player);
    }

    // 캐치마인트 플레이어 제거
    catchGamePlayerDelete(uuid: string) {
        return this.catchPlayerRepository.delete({ uuid });
    }

    // 무궁화 새로운 플레이어 추가
    redGreenGamePlayerInsert(player: RedGreenPlayer) {
        return this.redGreenPlayerRepository.insert(player);
    }

    // 무궁화 꽃이 플레이어 생성
    redGreenGamePlayerSave(player: RedGreenPlayer) {
        return this.redGreenPlayerRepository.save(player);
    }

    // 무궁화 꽃이 플레이어 제거
    redGreenGamePlayerDelete(uuid: string) {
        return this.redGreenPlayerRepository.delete({ uuid });
    }

    // 무궁화 꽃이 플레이어 조회
    redGreenGamePlayerFindByUuid(uuid: string) {
        return this.redGreenPlayerRepository.findOne({
            where: { uuid },
        });
    }

    redGreenGamePlayerFindAll(): Promise<RedGreenPlayer[]> {
        return this.redGreenPlayerRepository.find();
    }

    // 무궁화 꽃이 플레이어 정렬 조회
    redGreenGamePlayerSortByDistance(room_id: number): Promise<RedGreenPlayer[]> {
        return this.redGreenPlayerRepository.find({
            where: { room: { room_id } },
            order: {
                distance: 'ASC',
